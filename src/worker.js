...
async function uploadImage(request, env) {
  ...
  const response = await fetch(`https://api.github.com/repos/${REPO_OWNER}/${REPO_NAME}/contents/${path}`, {
    method: 'PUT', headers: githubHeaders(env.GITHUB_TOKEN), body: JSON.stringify({
      message: `Add resource image ${filename}`, content: toBase64(bytes), branch: DEFAULT_BRANCH,
  ...
  try {
    ...
    if (landing.ok || landing.status < 400) {
      ...
      if (htmlType.includes('text/html')) {
        ...
        if (match?.[1]) { try { return json({ logoUrl: new URL(match[1], landing.url).href }, 200, request); } catch {} }
      }
    }
  } catch {}
...
const inject = (response, script) => new HTMLRewriter().on('body', { element(element) { element.append(`<script src="${script}" defer></script>`, { html: true }); } }).transform(response);
...
export default {
  async fetch(request, env) {
    ...
    if (request.method === 'GET' && url.pathname.startsWith('/resources/') && contentType.includes('text/html')) return inject(response, '/resource-tool-logos.js');
    if (request.method === 'GET' && url.pathname.startsWith('/admin/') && contentType.includes('text/html')) return inject(response, '/admin/image-controls.js');
    return response;
  }
};
