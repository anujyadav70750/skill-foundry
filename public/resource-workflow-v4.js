import('/resource-workflow-v3.js').then(() => {
  const init = () => {
    ...
    const toolInfo=name=>{
      ...
      const node=nodes.find(n=>{const n1=n.dataset.toolName||n.querySelector('h3')?.textContent?.trim()||n.getAttribute('aria-label')?.replace(/^Open\\s+/i,'');return n1&&n1.toLowerCase()===wanted});
    ...
    const logoFor=async href=>{
      if(!href)return '';
      try{const r=await fetch('/api/tool-logo?url='+encodeURIComponent(href),{credentials:'same-origin',cache:'no-store'});if(r.ok){const d=await r.json();if(d?.logoUrl&&await validImage(d.logoUrl))return d.logoUrl}}catch{}
      try{const u=new URL(href,location.href);const candidates=[u.origin+'/favicon.svg',u.origin+'/favicon.png',u.origin+'/apple-touch-icon.png','https://www.google.com/s2/favicons?domain='+encodeURIComponent(u.hostname)+'&sz=128'];for(const src of candidates)if(await validImage(src))return src}catch{}
      return '';
    };
    const fixTools=async()=>{
      [...section.querySelectorAll('.sf-tool-badge')].forEach(async badge=>{
        if(badge.dataset.sfV5==='1')return;
        const name=badge.textContent?.trim();if(!name)return;
        badge.dataset.sfV5='1';const info=toolInfo(name);
        const row=document.createElement('div');row.className='sf-tool-row';
        const a=document.createElement('a');a.className='sf-tool-logo-link';a.target='_blank';a.rel='noopener noreferrer';a.title='Open '+name;a.setAttribute('aria-label','Open '+name);if(info.href)a.href=info.href;
        const img=document.createElement('img');img.alt='';img.width=38;img.height=38;img.loading='lazy';img.decoding='async';img.referrerPolicy='no-referrer';
        const fallback=()=>{if(!img.isConnected)return;img.remove();a.classList.add('is-fallback');a.textContent=name.slice(0,1).toUpperCase()};img.onerror=fallback;
        const src=await logoFor(info.href)||info.logo;if(src)img.src=src;else fallback();a.appendChild(img);
        const purpose=document.createElement('div');purpose.className='sf-tool-purpose';const label=document.createElement('span');label.className='sf-tool-purpose-label';label.textContent='TOOL USED FOR';const text=document.createElement('strong');text.className='sf-tool-purpose-text';text.textContent=info.purpose||'Used at this step to create the output.';purpose.append(label,text);
        row.append(a,purpose);badge.replaceWith(row);
      });
    };
