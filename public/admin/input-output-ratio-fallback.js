(() => {
  ...
  const target = el => el && el.tagName === 'INPUT' && el.type === 'file' && (el.getAttribute('data-file-role') === 'input' || el.getAttribute('data-file-role') === 'result');
  const host = input => input.closest('.media-item');
  const path = input => host(input)?.querySelector(`[data-role="${input.getAttribute('data-file-role')}"]`);
  ...
  const immediatePreview = (input, file) => {
    ...
    if (!p) return;
    const old = p.dataset.objectUrl;
    if (old) URL.revokeObjectURL(old);
    const url = URL.createObjectURL(file);
    p.dataset.objectUrl = url;
    p.classList.add('has-image');
    p.innerHTML = `<button class="image-clear" type="button" aria-label="Remove selected image" title="Remove selected image">×</button><img src="${url}" alt="Selected image preview"><span>${String(file.name).replace(/[&<>\"']/g, s => ({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;',"'":'&#039;'}[s]))}</span>`;
    p.querySelector('.image-clear')?.addEventListener('click', e => {
      e.preventDefault();
      e.stopPropagation();
  ...
  const showResult = (input, file, blob, ratio) => {
    ...
    upload.onclick=async()=>{ upload.disabled=true; upload.textContent='Uploading…'; msg.textContent='Uploading to website…'; try{
      const base=file.name.replace(/\.[^.]+$/,''); const rn=ratio==='original'?'original':ratio.replace(':','x'); const name=`${base}-${input.getAttribute('data-file-role')}-${rn}.jpg`;
  ...
  const open = (input,file) => {
    ...
    const role=input.getAttribute('data-file-role')==='input'?'input':'output';
    ...
    d.querySelector('.sf-cancel').onclick=()=>{URL.revokeObjectURL(url);close();};
    d.oncancel=e=>{e.preventDefault();URL.revokeObjectURL(url);close();};
