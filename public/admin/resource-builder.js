(() => {
  ...
  const listConfig = [
    ['inputs-list', 'input', addInput, 1], ['results-list', 'result', addResult, 1],
    ['tools-list', 'tool', addTool, 1], ['steps-list', 'step', addStep, 1],
    ['tips-list', 'tip', addTip, 1], ['tags-list', 'tag', addTag, 1], ['related-list', 'related', addRelated, 1]
  ];
  listConfig.forEach(([id, type, fn, minimum]) => {
    const list = document.querySelector('#' + id);
    if (list) list.dataset.minimum = String(minimum);
  });

  // Each Add button is moved directly below its repeat list.
  ...
  const ensureMinimums = () => {
    listConfig.forEach(([id, , fn, minimum]) => {
      const list = document.querySelector('#' + id);
      if (!list) return;
      while (list.children.length < minimum) fn();
    });
  };
  ...
  const renderPreview = () => {
    const d = readForm(); const r = d.repeat; const primary = r.tools[0];
    let html = `<h3>${escapeHtml(d.title || 'Untitled resource')}</h3><p class="preview-meta">${escapeHtml(d.category || 'Category')} · ${escapeHtml(primary?.name || 'Tool used')}${d.date ? ` · Published ${escapeHtml(d.date)}` : ''}</p>`;
    ...
    if (r.tips.length) html += `<h4>Tips for better results</h4><ul class="preview-list">${r.tips.map((x) => `<li>${escapeHtml(x)}</li>`).join('')}</ul>`;
    if (r.tags.length) html += `<h4>Tags</h4><p class="preview-copy">${escapeHtml(r.tags.join(', '))}</p>`;
    if (r.related.length) html += `<h4>Related resources</h4><ul class="preview-list">${r.related.map((x) => `<li>${escapeHtml(x)}</li>`).join('')}</ul>`;
  ...
  const restoreRepeats = (repeat = {}, legacy = {}) => {
    ...
    tools.forEach(addTool); (repeat.steps?.length ? repeat.steps : [{}]).forEach(addStep);
    (repeat.tips?.length ? repeat.tips : ['']).forEach(addTip); (repeat.tags?.length ? repeat.tags : ['']).forEach(addTag);
    (repeat.related?.length ? repeat.related : ['']).forEach(addRelated);
    placeAddButtons();
  };
  const writeForm = (data) => {
    form.querySelectorAll('input[name], textarea[name], select[name]').forEach((field) => { if (!(field.name in data)) return; if (field.type === 'checkbox') field.checked = Boolean(data[field.name]); else field.value = data[field.name] ?? ''; });
    slugManuallyEdited = Boolean(data.slug); restoreRepeats(data.repeat || {}, data);
  };
  ...
  document.querySelectorAll('[data-add]').forEach((button) => button.addEventListener('click', () => {
    const type = button.dataset.add; if (type === 'input') addInput(); if (type === 'result') addResult(); if (type === 'tool') addTool(); if (type === 'step') addStep(); if (type === 'tip') addTip(); if (type === 'tag') addTag(); if (type === 'related') addRelated();
    placeAddButtons(); renderPreview();
  }));
  document.querySelectorAll('[data-file-role]').forEach((file) => file.addEventListener('change', () => {
    const selected = file.files?.[0]; if (!selected) return; const role = file.dataset.fileRole; const input = form.elements[role]; if (input) input.value = `/images/${selected.name.replace(/[^a-zA-Z0-9._-]+/g, '-').toLowerCase()}`; showLocalPreview(document.querySelector(`[data-preview-role="${role}"]`), selected); renderPreview();
  }));
