import('/resource-workflow-v3.js').then(() => {
  const init = () => {
    ...
    if (!document.querySelector('#sf-workflow-v5-style')) {
      ...
      style.textContent = `
        ...
        @media(max-width:420px){
          ...
          .workflow-section.sf-v3 .sf-action-head { grid-template-columns:52px minmax(0,1fr) 62px!important; }
    ...
    const toolInfo = name => {
      ...
      return node ? {
        href: node.href || node.querySelector('.tool-link')?.href || '',
        purpose: node.dataset.toolPurpose || node.querySelector('p')?.textContent?.trim() || '',
    ...
    const logoFor = async href => {
      ...
      try {
        ...
        if (r.ok) { const d = await r.json(); if (d?.logoUrl && await validImage(d.logoUrl)) return d.logoUrl; }
      } catch {}
      try {
    ...
    const fixTools = async () => {
      [...section.querySelectorAll('.sf-tool-badge')].forEach(async badge => {
        ...
        const fallback=()=>{img.remove();a.classList.add('is-fallback');a.textContent=name.slice(0,1).toUpperCase();};
        img.onerror=fallback;
        const src=await logoFor(info.href)||info.logo;
        if(src){img.src=src;}else{fallback();}
        a.appendChild(img);
        const purpose=document.createElement('div'); purpose.className='sf-tool-purpose';
        const label=document.createElement('span'); label.className='sf-tool-purpose-label'; label.textContent='TOOL USED FOR';
        const text=document.createElement('strong'); text.className='sf-tool-purpose-text'; text.textContent=info.purpose||'Used at this step to create the output.';
        purpose.append(label,text); row.append(a,purpose); badge.replaceWith(row);
      });
    };
